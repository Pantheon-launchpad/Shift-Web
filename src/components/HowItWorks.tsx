...
export default function HowItWorks() {
  return (
    <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto bg-[#f8f9fa]">
      {/* Header */}
      <motion.div 
      ...
      >
        <motion.span 
          className="inline-flex items-center px-4 py-1.5 bg-white text-purple-600 rounded-full text-xs font-medium mb-4 border border-gray-200 shadow-sm"
        ...
        <motion.h2 
          className="text-4xl sm:text-[42px] font-bold text-gray-900 mb-4 leading-tight tracking-tight"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {steps.map((step, idx) => (
          ...
          >
            ...
            {/* Visual Content */}
            <div className="h-44 relative overflow-hidden bg-gray-50/30 rounded-xl">
              {step.type === 'connect' && (
                <div className="relative w-full h-full">
                  {/* Concentric circles - very subtle */}
                  <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 176">
                    <motion.ellipse
                      cx="100"
                      cy="88"
                      rx="85"
              ...
              {step.type === 'track' && (
                <div className="relative w-full h-full flex flex-col items-center justify-end pb-4">
                  ...
                  >
                    <span className="text-[10px] text-gray-600 font-medium">Low Engagement</span>
                    <span className="text-[9px] font-bold text-orange-600 bg-orange-100 px-1.5 py-0.5 rounded">-8%</span>
                  </motion.div>
                  ...
                  >
                    <span className="text-[10px] text-gray-600 font-medium">High Engagement</span>
                    <span className="text-[9px] font-bold text-purple-600 bg-purple-100 px-1.5 py-0.5 rounded">+12%</span>
                  </motion.div>
              ...
              {step.type === 'action' && (
                <div className="relative w-full h-full flex items-end justify-center pb-2">
                  ...
                  <motion.div
                    className="absolute top-2 left-1/2 -translate-x-1/2 bg-white rounded-md px-3 py-1 shadow-md border border-gray-100 z-20"
                    initial={{ y: 15, opacity: 0 }}
                    whileInView={{ y: 0, opacity: 1 }}
                    transition={{ delay: 0.5 }}
                  >
                    <span className="text-[10px] text-purple-500 font-medium">Optimize</span>
                  </motion.div>

                  <motion.div
                    className="absolute top-10 left-8 bg-white rounded-md px-2.5 py-1 shadow-md border border-gray-100 transform -rotate-12 z-20"
                    initial={{ x: -15, opacity: 0 }}
                    whileInView={{ x: 0, opacity: 1 }}
                    transition={{ delay: 0.6 }}
                  >
                    <span className="text-[9px] text-purple-500 font-bold">Action</span>
                  </motion.div>

                  <motion.div
                    className="absolute top-12 left-24 bg-white rounded-md px-2.5 py-1 shadow-md border border-gray-100 transform rotate-6 z-20"
                    initial={{ y: 15, opacity: 0 }}
                    whileInView={{ y: 0, opacity: 1 }}
                    transition={{ delay: 0.7 }}
                  >
                    <span className="text-[9px] text-gray-500">Fix churn spike</span>
                  </motion.div>

                  <motion.div
                    className="absolute top-10 right-8 bg-white rounded-md px-2.5 py-1 shadow-md border border-gray-100 transform rotate-12 z-20"
                    initial={{ x: 15, opacity: 0 }}
                    whileInView={{ x: 0, opacity: 1 }}
                    transition={{ delay: 0.8 }}
                  >
                    <span className="text-[9px] text-gray-700 font-bold">$30,982</span>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gray-100 rounded-lg overflow-hidden">
                      {/* Bottom part */}
                      <div className="absolute inset-x-0 bottom-0 h-16 bg-gray-50" />
                      
                      {/* Side flaps */}
                      <div 
                        className="absolute bottom-0 left-0 w-22 h-16 bg-gray-100"
                        style={{ clipPath: 'polygon(0 100%, 100% 0, 0 0)' }}
                      />
                      <div 
                        className="absolute bottom-0 right-0 w-22 h-16 bg-gray-100"
